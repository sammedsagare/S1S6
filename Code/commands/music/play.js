...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (message.guild.me.voice.channel && message.member.voice.channel.id != message.guild.me.voice.channel.id) return message.channel.send(" You must join my voice channel: " + ` \`${message.guild.me.voice.channel.name ? message.guild.me.voice.channel.name : ""}\``)
    ...
    //do things for spotify playlist
      ...
      let tracklength = items.length;
      if (tracklength > 25) {
        message.reply("the current maximum of tracks for spotify playlists are 25 tracks, now playing the first 25 songs!");
        tracklength = 25;
      }
      functions.embedbuilder(client, 5000, message, config.colors.yes, "🎶  Fetching the songs!", "This will take me around about: " + tracklength / 2 + " seconds");
      for (let i = 0; i < 25; i++) {
